...
MOVE_SPEED = 5.0
MAX_HEALTH = 100
SHOOT_HIT_ANGLE_COS = math.cos(math.radians(8))
...
def vec_len(v): return math.sqrt(sum([x*x for x in v]))
def normalize(v): L=vec_len(v); return (0,0,0) if L==0 else tuple(x/L for x in v)
def rotate_yaw_forward(yaw,amt=1.0): return (math.sin(yaw)*amt, math.cos(yaw)*amt)
...
async def physics_tick(dt):
    ...
    for pid,p in players.items():
        ...
        if keys.get("w"): dx+=fwd[0]; dz+=fwd[1]
        if keys.get("s"): dx-=fwd[0]; dz-=fwd[1]
        if keys.get("a"): dx-=rgt[0]; dz-=rgt[1]
        if keys.get("d"): dx+=rgt[0]; dz+=rgt[1]
        L=math.hypot(dx,dz)
        if L>0: dx=dx/L*MOVE_SPEED*dt; dz=dz/L*MOVE_SPEED*dt
        p["x"]=max(-B,min(B,p["x"]+dx)); p["z"]=max(-B,min(B,p["z"]+dz))
...
async def handle_message(ws,msg):
    ...
    if mtype=="input":
        ...
        if now()-last_shot<weapon["fire_rate"]: return
        if p.get("ammo",0)<=0: return
        p["ammo"]-=1
        p["last_shot"]=now()
        ...
        if wid in weapons:
            ...
            await notify_player_byid(pid,{"type":"weapon_pickup","weapon":wp["type"],"ammo":p["ammo"],"id":wid})
...
async def register(ws,info):
    ...
    clients[ws]=pid
    sx,sz=(len(players)%4)*2.0,(len(players)//4)*2.0
    players[pid]={"id":pid,"x":sx,"y":1.0,"z":sz,"yaw":0,"pitch":0,"health":MAX_HEALTH,"keys":{}}
...
async def unregister(ws):
    ...
    if pid: players.pop(pid,None); await notify_all({"type":"leave","id":pid})

# HANDLER
async def handler(ws):
    try:
        intro=await asyncio.wait_for(ws.recv(),timeout=5)
        try:intro_j=json.loads(intro)
        except:intro_j={}
        await register(ws,intro_j)
    except: await register(ws,{})
    try:
        async for msg in ws:
    ...
    finally: await unregister(ws)

# WEAPON SPAWN LOOP
async def spawn_weapon_loop():
    while True:
        await asyncio.sleep(WEAPON_SPAWN_INTERVAL)
        if len(weapons)>=WEAPON_MAX: continue
        wid=str(uuid.uuid4())[:8]
        wtype=random.choice(list(WEAPON_DATA.keys()))
        weapons[wid]={"id":wid,"type":wtype,"x":random.uniform(-50,50),"y":1.0,"z":random.uniform(-50,50)}
    ...
    while True:
        ...
        for proj in projectiles:
            ...
            for oid,op in players.items():
                if oid==proj["owner"] or op["health"]<=0: continue
                dx=op["x"]-proj["pos"][0]; dy=op["y"]-proj["pos"][1]; dz=op["z"]-proj["pos"][2]
                if math.sqrt(dx*dx+dy*dy+dz*dz)<1.0:
                    damage=WEAPON_DATA[proj["type"]]["damage"]
                    op["health"]=max(0,op["health"]-damage)
                    await notify_player_byid(oid,{"type":"got_hit","by":proj["owner"],"health":op["health"]})
                    await notify_player_byid(proj["owner"],{"type":"hit","target":oid,"by":proj["owner"],"health":op["health"]})
                    if op["health"]==0: await notify_all({"type":"kill","killer":proj["owner"],"victim":oid})
                    remove.append(proj)
                    break
            if now()-proj["time"]>2.0: remove.append(proj)
        for r in remove: projectiles.remove(r)
...
if __name__=="__main__":
    ...
    print("Starting FPS server on ws://0.0.0.0:8765")
    asyncio.run(main())
